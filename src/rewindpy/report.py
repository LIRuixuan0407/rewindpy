...
function normalizePayload(raw){if(!raw||typeof raw!=='object'||Array.isArray(raw))throw new Error('report payload must be an object');const schema=raw.schema_version==null?1:Number(raw.schema_version);if(!Number.isInteger(schema)||schema<1)throw new Error('schema_version must be a positive integer');if(schema>CURRENT_REPORT_SCHEMA)throw new Error(`unsupported report schema ${schema}; this viewer supports up to ${CURRENT_REPORT_SCHEMA}`);const object=value=>value&&typeof value==='object'&&!Array.isArray(value);if(schema===CURRENT_REPORT_SCHEMA){if(typeof raw.rewindpy_version!=='string'||!raw.rewindpy_version)throw new Error('rewindpy_version must be a non-empty string');if(!Array.isArray(raw.events))throw new Error('events must be an array');if(!object(raw.sources))throw new Error('sources must be an object');if(!object(raw.crash))throw new Error('crash must be an object');if(!object(raw.trace_stats))throw new Error('trace_stats must be an object')}return{...raw,schema_version:CURRENT_REPORT_SCHEMA,events:Array.isArray(raw.events)?raw.events:[],sources:object(raw.sources)?raw.sources:{},crash:object(raw.crash)?raw.crash:{},trace_stats:object(raw.trace_stats)?raw.trace_stats:{},crash_slice:object(raw.crash_slice)?raw.crash_slice:{steps:[]}}}
